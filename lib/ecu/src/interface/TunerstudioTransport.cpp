...
static uint32_t crc32inc(TContainer buf)
{
	uint32_t crc = 0;
	...
	while (size-- > 0)
	{
		crc = crc32_tab[(crc ^ buf[idx++]) & 0xFF] ^ (crc >> 8);
	}
...
void TunerstudioProtocol::WriteCrcPacket(std::vector<uint8_t>& data)
{
	Span<uint8_t> sdata(data.data(), data.size());
...
std::vector<uint8_t> TunerstudioProtocol::ReadCrcPacket()
{
	// Read the header
	uint8_t header[2];
	while (0 == m_port->Read(header, sizeof(header)));

	const size_t packetLength =
		(header[0] << 8 | header[1]);

	const size_t packetLengthWithCrc = packetLength + 4;	// 4 extra bytes for crc
	
	std::vector<uint8_t> response(packetLengthWithCrc);

	size_t idx = 0;

	// Read until we get enough bytes - may take multiple due to intermediate buffer sizes
	while (idx < packetLengthWithCrc)
	{
		size_t read = m_port->Read(response.data() + idx, response.size() - idx);

		idx += read;
	}
	
	Span<uint8_t> responseNoCrc(response.data(), packetLength);
	Span<uint8_t> responseCrc(response.data() + packetLength, 4);
...
std::vector<uint8_t> TunerstudioProtocol::CrcCommand(char command)
{
	std::vector<uint8_t> req{ 0, 1, static_cast<uint8_t>(command) };
	return DoTransactionCrc(req);
}

std::vector<uint8_t> TunerstudioProtocol::DoTransactionCrc(std::vector<uint8_t>& command)
{
	WriteCrcPacket(command);
	return ReadCrcPacket();
}
}
